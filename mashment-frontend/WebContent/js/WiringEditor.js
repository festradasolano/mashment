(function() {
			...
			.extend(
					...
					YAHOO.util.DDProxy,
					{
						...
						startDrag : function(e) {
							WireIt.ModuleProxy.superclass.startDrag.call(this,
									e);
						...
						onDragDrop : function(e, ddTargets) {
							...
							var layerTarget = ddTargets[0], layer = ddTargets[0]._layer, del = this
									.getDragEl(), pos = YAHOO.util.Dom
									.getXY(del), layerPos = YAHOO.util.Dom
									.getXY(layer.el);
							this._WiringEditor
	...
	WireIt.WiringEditor.prototype = {
		...
		setOptions : function(options) {
			...
			this.options.layerOptions.parentEl = layerOptions.parentEl ? layerOptions.parentEl
					: Dom.get('wiring-editor');
			this.options.layerOptions.layerMap = YAHOO.lang
					.isUndefined(layerOptions.layerMap) ? true
			...
			this.options.accordionViewParams = options.accordionViewParams || {
				collapsible : true,
				expandable : true, // remove to open only one panel at a time
				width : '308px',
				expandItem : 0,
				animationSpeed : '0.3',
				animate : true,
		...
		renderAccordion : function() {
			this.accordionView = new YAHOO.widget.AccordionView(
					'accordionView', this.options.accordionViewParams);
		},
		...
		addModule : function(module, pos) {
			try {
				...
				Dom
						.addClass(container.el, "WiringEditor-module-"
								+ module.name);
			} catch (ex) {
		...
		saveModuleSuccess : function(o) {
			 ...
			 * if(this.modulesByName.hasOwnProperty(name) ) { //already exists }
			 * else { //new one }
			 */
		...
		onNew : function() {
			...
			// false to tell inputEx to NOT send the updatedEvt
		...
		renderLoadPanel : function() {
			if (!this.loadPanel) {
				...
				this.loadPanel
						.setBody("Filter: <input type='text' id='loadFilter' /><div id='loadPanelBody'></div>");
				this.loadPanel.render(document.body);

				// Listen the keyup event to filter the module list
				Event.onAvailable('loadFilter', function() {
					Event.addListener('loadFilter', "keyup",
							this.inputFilterTimer, this, true);
				}, this, true);
		...
		/**
		 * Method called from each keyup on the search filter in load panel. The
		 * real filtering occurs only after 500ms so that the filter process
		 * isn't called too often
		 */
		...
		updateLoadPanelList : function(filter) {
			...
			if (lang.isArray(this.pipes)) {
				for (var i = 0; i < this.pipes.length; i++) {
					...
					if (!filter || filter === ""
							|| module.name.match(new RegExp(filter, "i"))) {
			...
			for (var i = 0; i < n; i++) {
				if (this.pipes[i].name == name) {
					...
					try {
						...
						this.alert("Unable to eval working json for module "
								+ name);
						return null;
					}
				}
		...
		loadPipe : function(name) {
			...
			try {
				...
				// the false tells inputEx to NOT fire the updatedEvt
				this.propertiesForm.setValue(wiring.properties, false);
				...
				if (lang.isArray(wiring.modules)) {
					...
					for (i = 0; i < wiring.modules.length; i++) {
						...
						if (this.modulesByName[m.name]) {
							...
							YAHOO.lang.augmentObject(m.config,
									baseContainerConfig);
							m.config.title = m.name;
							var container = this.layer.addContainer(m.config);
							Dom.addClass(container.el, "WiringEditor-module-"
									+ m.name);
							container.setValue(m.value);
						} else {
							throw new Error("WiringEditor: module '" + m.name
									+ "' not found !");
						}
					}
					...
					if (lang.isArray(wiring.wires)) {
						for (i = 0; i < wiring.wires.length; i++) {
							// On doit chercher dans la liste des terminaux de
							// chacun des modules l'index des terminaux...
		...
		renderAlertPanel : function() {
			...
			this.alertPanel
					.setBody("<div id='alertPanelBody'></div><button id='alertPanelButton'>Ok</button>");
		...
		/**
		 * This method return a wiring within the given vocabulary described by
		 * the modules list
		 * 
		 * @method getValue
		 */
		getValue : function() {
			...
			for (i = 0; i < this.layer.wires.length; i++) {
				...
				var wireObj = {
					src : {
						moduleId : WireIt.indexOf(wire.terminal1.container,
								this.layer.containers),
						terminal : wire.terminal1.options.name
					},
					tgt : {
						moduleId : WireIt.indexOf(wire.terminal2.container,
								this.layer.containers),
